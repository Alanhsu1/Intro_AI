...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        bestError=float('inf') # init to max
        for f in range(len(featureVals)): # equals to len(features)
            tmp=list()
            for d in range(len(featureVals[0])): # equals to len(dataset)
                if featureVals[f][d] < 0.0:
                    tmp.append(1.0)
                else: tmp.append(0.0)
            
            err=0.0
            for i in range(len(tmp)):
                err = err + weights[i]*abs(tmp[i]-labels[i])
            if err < bestError:
                bestError = err  
                bestClf = WeakClassifier(features[f])
                # output is the classfier with lowest error
